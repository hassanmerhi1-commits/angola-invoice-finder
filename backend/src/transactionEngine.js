 ...
 * 
 * Every business operation flows through this engine.
 * It ensures atomic updates to: Stock Movements, Journal Entries, Open Items, Balances.
 * 
 * This is the SAP-equivalent "Document Flow into Accounting" processor.
 */
const db = require('./db');
...
/**
 * Get current stock for a product at a warehouse (from movements)
 */
...
/**
 * Create an open item (when an invoice is issued)
 */
...
/**
 * Clear open items (match payment against invoices)
 * Returns the clearing records created
 */
async function clearOpenItems(client, params) {
  ...
  for (let i = 0; i < invoiceItemIds.length; i++) {
    ...
    // Update invoice open item remaining
    await client.query(
      `UPDATE open_items SET 
       remaining_amount = remaining_amount - $1,
       status = CASE 
         WHEN remaining_amount - $1 <= 0.01 THEN 'cleared'
         ELSE 'partial'
       END,
    ...
    // Update payment open item remaining
    await client.query(
      `UPDATE open_items SET 
       remaining_amount = remaining_amount - $1,
       status = CASE 
...
// ==================== CENTRAL TRANSACTION PROCESSORS ====================

/**
 * PROCESS SALE — The master function for sales
 * 1. Validate period
 * 2. Insert sale + items
 * 3. Record stock movements (OUT) for each item
 * 4. Create journal entries (Revenue + COGS)
 * 5. Create open item (if credit sale) or auto-clear (if cash)
 * 6. Update Caixa session
 */
...
/**
 * PROCESS PURCHASE RECEIVE — When goods arrive
 * 1. Validate period
 * 2. Update PO status
 * 3. Record stock movements (IN) with WAC calculation
 * 4. Create journal entries (Inventory + Supplier Payable)
 * 5. Create open item for supplier
 * 6. Link PO → Goods Receipt
 */
async function processPurchaseReceive(client, orderId, receivedQuantities, receivedBy) {
  ...
  for (const item of itemsResult.rows) {
    ...
    if (receivedQty > 0) {
      ...
      if (productResult.rows.length > 0) {
        ...
        // Update product cost (stock updated by recordStockMovement)
        await client.query(
  ...
  // Open item for supplier
  if (order.supplier_id) {
    await createOpenItem(client, {
      entityType: 'supplier',
      entityId: order.supplier_id,
      documentType: 'invoice',
      documentId: orderId,
      documentNumber: order.order_number,
      documentDate: today,
      dueDate: null, // Could calculate from supplier payment terms
      originalAmount: subtotal + freightCost + taxAmount,
      isDebit: true,
      branchId: order.branch_id,
    });
  }
 ...
 * 1. Record stock movements (OUT from source, IN to destination)
 * 2. Journal entry (internal movement)
 * 3. Link documents
 */
async function processTransferReceive(client, transferId, receivedQuantities, receivedBy) {
  ...
  for (const item of itemsResult.rows) {
    ...
    if (receivedQty > 0) {
      ...
      await recordStockMovement(client, {
        productId: item.product_id,
        warehouseId: transfer.to_branch_id,
        movementType: 'IN',
        quantity: receivedQty,
        unitCost,
        ...
        notes: `De ${transfer.from_branch_name}`,
        createdBy: receivedBy,
      });
    }
  }
...
async function processTransferApprove(client, transferId, approvedBy) {
  ...
  for (const item of itemsResult.rows) {
    ...
    await recordStockMovement(client, {
      ...
      referenceId: transferId,
      referenceNumber: transfer.transfer_number,
      notes: `Para ${transfer.to_branch_name}`,
  ...
  console.log(`[TX ENGINE] Transfer ${transfer.transfer_number}: Approved, Stock OUT from ${transfer.from_branch_name} ✓`);
  return transfer;
}

/**
 * PROCESS PAYMENT — Record payment and clear open items
 */
async function processPayment(client, paymentData) {
  ...
  // Create open item for the payment (credit side)
  ...
  // Auto-clear against specified invoices
